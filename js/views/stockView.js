// Stock View - Enhanced with image upload feature
...
export class StockView {
        ...
        return `
            <div class="stock-item" data-product-id="${product.id}">
                ...
                <div class="stock-item-actions">
                    ...
                    ${isAdmin ? `
                        <button class="stock-action-btn edit" data-action="edit-image" data-product-id="${product.id}" title="เปลี่ยนรูปภาพ">
                            <svg viewBox="0 0 24 24">
    ...
    async handleStockAction(button) {
        ...
        switch (action) {
            ...
            case 'adjust':
                ...
                break;
            case 'edit-image':
                await this.showImageUploadModal();
                break;
            case 'edit-product':
    ...
    async showImageUploadModal() {
        const imageUrl = await this.modal.prompt(
            'เปลี่ยนรูปภาพสินค้า',
            `ใส่ URL รูปภาพสำหรับ "${this.currentProduct.name}"`,
        ...
        if (imageUrl !== null) {
            try {
                ...
                await this.productService.updateProduct(this.currentProduct.id, {
                    image_url: imageUrl || null
                });
                
                // Update local data
                this.currentProduct.image_url = imageUrl || null;
                const productIndex = this.products.findIndex(p => p.id === this.currentProduct.id);
                if (productIndex !== -1) {
                    this.products[productIndex].image_url = imageUrl || null;
                }
                ...
                this.toast.error('เกิดข้อผิดพลาดในการอัปเดตรูปภาพ');
            }
        }
    }
